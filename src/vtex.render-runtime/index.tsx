...
function validateRootComponent(rootName: string, extensions: any) {
...
function start() {
  try {
    ...
    if (!canUseDOM) {
      ...
      >).then(({ markups, maxAge, page, renderTimeMetric, ...rendered }: any) => ({
        extensions: markups.reduce(
          (acc, { name, markup }) => {(acc[name] = markup); return acc},
          {}
        ),
        head: Helmet.rewind(),
        maxAge,
