...
function getPagePath(name: string, pages: any) {
  const { path: pagePath, cname } = pages[name]
  return cname && isHost(cname) ? '/' : pagePath
}

function checkValidParams(id: string, pages: any, path: string, params: any) {
...
export function scrollTo(options: any) {
...
function polyfillScrollTo(options: any) {
  try {
...
function routeMatchFromPath(path: string, routes: any): RouteMatch | null {
  let id: string | undefined
