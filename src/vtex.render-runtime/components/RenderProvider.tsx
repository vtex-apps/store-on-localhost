...
export interface RenderProviderState {
  appsEtag: any['appsEtag']
  cacheHints: any['cacheHints']
  components: any['components']
  culture: any['culture']
  defaultExtensions: any['defaultExtensions']
  device: any
  ...
  blocksTree?: any['blocksTree']
  blocks?: any['blocks']
  contentMap?: any['contentMap']
}
...
class RenderProvider extends Component<Props, RenderProviderState> {
  ...
  public sendInfoFromIframe = debounce(
    (params?: { shouldUpdateRuntime?: boolean }) => {
      ...
      return (window as any).top.__provideRuntime(
  ...
  public constructor(props: Props) {
    ...
    if (history) {
      ...
      if (!equals(history.location as any, renderLocation)) {
        history.replace(renderLocation)
      }
      // backwards compatibility
      window.browserHistory = (global as any).browserHistory = history
    }
  ...
  public scrollTo = (scrollOptions?: any) => {
    try {
  ...
  public onPageChanged = (location: any) => {
    const {
      runtime: { renderMajor },
    } = this.props
    const {
    ...
    const query = queryStringToMap(location.search) as any['query']
  ...
  public updateComponentAssets = (availableComponents: any) => {
    this.setState({
      components: {
  ...
  public updateRuntime = async (options?: any) => {
    const {
      runtime: { renderMajor },
    } = this.props
    const {
  ...
  public updateExtension = async (name: string, extension: any) => {
    const { extensions } = this.state
  ...
  public handleSetDevice = (device: any) => {
    this.setState({ device })
  }

  public addMessages = async (newMessages: any['messages']) => {
    await new Promise<void>(resolve => {
      this.setState(
        state => ({
          ...state,
