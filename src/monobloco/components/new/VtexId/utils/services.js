...
export const redirect = ({ returnUrl }) => {
  ...
  window.location.href = Paths.redirect(absoluteReturnUrl.href)
}

export const redirectOAuth = ({ provider }) =>
  provider && (window.location.href = Paths.getOAuthRedirectUrl(provider))
...
export const logout = ({ parentAppId }) =>
  http.get(
    Paths.logout(encodeURIComponent(window.location.host)),
    getReqConfig(parentAppId)
  )
