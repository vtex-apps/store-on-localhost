...
>> = ({
  ...
  const showLastSearched =
    !searching && (lastSearched as any)?.value?.length > 0

  const getShowedOptions = (): AutocompleteOption[] => {
    if (showLastSearched) {
      return (lastSearched as any)?.value
    }
    if (searching) {
      return options
    }
  ...
  const addToLastSearched = (option: AutocompleteOption) => {
    (lastSearched as any)?.onChange(option)
  }
  ...
  const getOptionProps = (option: any, index: any) => ({
    ...
    <div className="flex flex-column">
      {showLastSearched ? (
        <div className="pa4 b f6">
          {(lastSearched as any)?.label || 'Last searched terms'}
        </div>
      ) : null}
